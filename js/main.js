export default class Main {
  constructor() {
    ...
    this.currentScene = 'wheel'; // 默认场景为转盘
  }
  ...
  drawCenterButton() {
    ...
    // 绘制外圈圆形
    ctx.beginPath();
    ctx.arc(this.centerX, this.centerY, this.radius * 0.15, 0, Math.PI * 2);
    ...
    // 绘制内圈圆形
    ctx.beginPath();
    ctx.arc(this.centerX, this.centerY, this.radius * 0.13, 0, Math.PI * 2);
    ctx.strokeStyle = '#1E90FF';
    ctx.stroke();
    ...
    ctx.fillText('GO', this.centerX, this.centerY);

    // 绘制箭头
    const arrowSize = this.radius * 0.04;
    ctx.beginPath();
    ctx.moveTo(this.centerX, this.centerY - this.radius * 0.15 - arrowSize); // 箭头顶点
    ctx.lineTo(this.centerX - arrowSize, this.centerY - this.radius * 0.15); // 箭头左边
    ctx.lineTo(this.centerX + arrowSize, this.centerY - this.radius * 0.15); // 箭头右边
    ctx.closePath();
    ctx.fillStyle = '#1E90FF';
    ctx.fill();
  }
  ...
  bindTouchEvents() {
    wx.onTouchStart((e) => {
      ...
      if (touch.clientY >= tabY) {
          const tabWidth = this.canvas.width / 2; // 假设有两个选项
          const tabIndex = Math.floor(touch.clientX / tabWidth);

          if (tabIndex === 0) {
              this.currentScene = 'wheel'; // 转盘场景
              this.drawWheel(); // 绘制转盘
          } else if (tabIndex === 1) {
              this.currentScene = 'breakfast'; // 早餐转盘场景
              this.drawBreakfastWheel(); // 绘制早餐转盘
          }
          return;
      }
  ...
  drawList() {
    ...
    const items = [
        { icon: '👆', text: '自定义转盘' },
        { icon: '🥣', text: '吃什么早餐', onClick: () => this.navigateToBreakfastWheel() },
        { icon: '👍', text: '大冒险转盘' },
        { icon: '💭', text: '真心话转盘' },
        { icon: '❓', text: '真心话与大冒险' },
        { icon: '🎮', text: '峡谷64游戏转盘' },
    ...
    this.canvas.addEventListener('touchstart', (e) => {
        ...
        items.forEach((item, index) => {
            const itemY = 160 + index * 60;
            if (x > 15 && x < this.canvas.width - 15 && y > itemY && y < itemY + 50) {
  ...
  navigateToBreakfastWheel() {
    if (typeof wx.navigateTo === 'function') {
        wx.navigateTo({
            url: '/pages/breakfast-wheel/index'
        });
    } else {
        console.error('wx.navigateTo is not available in the current environment.');
        // 提供一个替代方案或抛出一个错误
    }
  }
  ...
  drawBreakfastWheel() {
    ...
    const options = [
        ...
        { text: '包子', color: '#5C7A7D' },
        { text: '粥', color: '#5E9E63' },
        { text: '油条', color: '#9E8E5E' },
        { text: '煎饼', color: '#8B4513' }
    ];
    ...
    options.forEach((option, index) => {
        ...
        ctx.fillStyle = '#ffffff';
        ctx.textAlign = 'center';
        ctx.fillText(option.text, this.centerX + Math.cos((startAngle + endAngle) / 2) * (this.radius * 0.5), this.centerY + Math.sin((startAngle + endAngle) / 2) * (this.radius * 0.5));
    });
